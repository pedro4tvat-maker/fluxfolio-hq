...
ALTER TABLE public.transactions ADD COLUMN IF NOT EXISTS os_code TEXT;
ALTER TABLE public.receivables  ADD COLUMN IF NOT EXISTS os_code TEXT;

CREATE INDEX IF NOT EXISTS idx_transactions_company_os ON public.transactions(company_id, os_code);
CREATE INDEX IF NOT EXISTS idx_receivables_company_os  ON public.receivables(company_id, os_code);
...
GRANT EXECUTE ON FUNCTION public.next_os_code(uuid, uuid) TO authenticated;

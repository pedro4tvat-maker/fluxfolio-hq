...
import { ArrowDownCircle, Building2, Search } from "lucide-react";
...
function ContasAReceber() {
  ...
  const { data = [], isLoading } = useQuery<ReceivableItem[]>({
    queryKey: ["receivables", selected],
    enabled: !!selected,
    queryFn: async () => {
      if (!selected) return [];
      ...
      return (receivables ?? []) as ReceivableItem[];
    },
  });
  ...
  if (!companies.length) {
    return (
      <div className="max-w-2xl mx-auto bg-card border rounded-2xl p-10 text-center shadow-card">
        ...
        <p className="text-sm text-muted-foreground mt-1">Selecione uma empresa para gerenciar as contas a receber.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-7xl">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Acompanhe os recebimentos planejados e vencidos.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="secondary"><ArrowDownCircle className="size-4" /> Novo recebimento</Button>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="bg-card border rounded-2xl p-5 shadow-card">
          ...
          <div className="font-display font-bold text-2xl mt-2 text-warning-foreground">{formatMoney(overdue)}</div>
        </div>
      </div>
      ...
      <div className="bg-card border rounded-2xl p-4 shadow-card flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[220px]">
          ...
          <Input value={query} onChange={(e) => setQuery(e.target.value)} className="pl-9" placeholder="Descrição do recebimento..." />
        </div>
      </div>

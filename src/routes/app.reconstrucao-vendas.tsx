    ...
    try {
      ...
      const rows = valid.map((it) => {
        ...
        return {
          ...
          quantity: qtd,
          unit_price: temPreco ? preco : 0,
          unit_cost: isService ? 0 : (temCusto ? custo : 0),
          total_revenue: temPreco ? receita : 0,
          total_cost: isService ? 0 : (temCusto ? custoTotal : 0),
          margin_value: !isService && temPreco && temCusto ? receita - custoTotal : (isService && temPreco ? receita : 0),
          margin_percentage: !isService && temPreco && temCusto && receita > 0 ? ((receita - custoTotal) / receita) * 100 : (isService && temPreco ? 100 : 0),
      ...
      for (const [, group] of grouped) {
        ...
        const itemRows = validItems.map((it) => {
          ...
          return {
            ...
            unit_price: temPreco ? preco : 0, unit_cost: temCusto ? custo : 0,
            total_revenue: temPreco ? qtd * preco : 0, total_cost: temCusto ? qtd * custo : 0,
            margin_value: temPreco && temCusto ? qtd * preco - qtd * custo : 0,
            margin_percentage: temPreco && temCusto && qtd * preco > 0 ? ((qtd * preco - qtd * custo) / (qtd * preco)) * 100 : 0,
